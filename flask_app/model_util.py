...
class DeepModel:
    ...
    def preprocess_image(path):
        ...
        '''
        img = process_image.image_utils.load_img(path, target_size=(224, 224))
        x = process_image.image_utils.img_to_array(img)
    ...
    def cosine_distance(self, input1, input2):
        ...
        return np.dot(input1, input2.T) / \
               np.dot(np.linalg.norm(input1, axis=1, keepdims=True), np.linalg.norm(input2.T, axis=0, keepdims=True))
